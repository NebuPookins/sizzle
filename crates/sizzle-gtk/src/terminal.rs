...
use gtk4::{
    ...
    EventControllerScrollFlags, GestureClick, GestureDrag, Popover, Scrollbar,
};
...
impl TerminalWidget {
    pub fn new(working_dir: Option<&str>, command: Option<String>) -> Self {
        ...
        let widget = Self {
            ...
            cols: cols_a,
            rows: rows_a,
            on_exit: dirty_flag.on_exit.clone(),
            adjustment,
            last_activity,
                ...
                match kv {
                    gdk::Key::Page_Up if page_lines > 0 => {
                        ...
                        let (doff, history) = {
                            ...
                            (
                                g.display_offset(),
                                g.total_lines().saturating_sub(g.screen_lines()) as f64,
                            )
                        };
            ...
            // Jump to bottom on any key press if scrolled back
            {
                ...
                if t.grid().display_offset() > 0 {
                    let hist =
                        t.grid()
                            .total_lines()
                            .saturating_sub(t.grid().screen_lines()) as f64;
                    drop(t);
    ...
    pub fn connect_key_pressed_capture<F: Fn(gdk::Key, gdk::ModifierType) -> bool + 'static>(
        &self,
        f: F,
    ) {
        let ctrl = EventControllerKey::new();
...
fn draw_term(
    ...
    for ic in &cells {
        ...
        if italic {
            f.set_style(pango::Style::Italic);
        }
