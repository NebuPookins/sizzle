...
pub fn scan_projects(settings: &crate::ScanSettings) -> Vec<ScannedProject> {
    let tags = TagDetector;
    let manual_roots: std::collections::HashSet<String> =
        settings.manual_project_roots.iter().cloned().collect();
    let mut results: Vec<ScannedProject> = Vec::new();

    for root in &settings.scan_roots {
        scan_dir(Path::new(root), &settings.ignore_roots, &manual_roots, &mut results, &tags);
    }

    // manual roots not already discovered by scan
    for manual in &settings.manual_project_roots {
        if !results.iter().any(|p| &p.path == manual) {
            let path = Path::new(manual);
            if path.exists() {
                let entries: Vec<_> = fs::read_dir(path)
                    .ok().map(|r| r.filter_map(|e| e.ok()).collect())
                    .unwrap_or_default();
                let readme_files = collect_readme_files(path, &entries);
                results.push(ScannedProject {
                    name: path.file_name().unwrap_or_default()
                        .to_string_lossy().to_string(),
                    path: manual.clone(),
                    readme_files,
                    detected_tags: tags.detect(path),
                });
            }
        }
    }
